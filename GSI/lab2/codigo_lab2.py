...
def processar_sensor_luva(leitura):
    ...
    # --- CEP: DETECÇÃO DE ESPASMO (Padrão Complexo) ---
    # Se a força subir mais de 5N em menos de 0.5 segundos
    ...
    return "OK"
    # --- SIMULAÇÃO DE FLUXO DE SENSORES (Gatilhos) ---
    fluxo_sensores = [
        {'sensor': 'Indicador', 'valor': 2.0},
        {'sensor': 'Indicador', 'valor': 4.5},
        {'sensor': 'Indicador', 'valor': 10.2}, # Subida brusca (>5N) -> Espasmo
        {'sensor': 'Indicador', 'valor': 16.0}, # Excedeu limite total
    ]
    for dado in fluxo_sensores:
        ...
        break
    time.sleep(0.5) # Latência de leitura do sensor