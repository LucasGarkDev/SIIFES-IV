...
def processar_transacao(evento):
    ...
    if not evento.get('vip', False):
        ...
        if len(compras_anteriores) >= 2:
            print(f" [ALERTA CEP]: Cliente {evento['cliente']} com múltiplas transações rápidas.")
    else:
        print(" [VIP]: Cliente VIP - isento da regra de fraude.")
...
fluxo_entrada = [
    ...
    {'cliente': 'Carlos', 'valor': 3000.00, 'vip': True},
    {'cliente': 'Carlos', 'valor': 1500.00, 'vip': True},
    {'cliente': 'Carlos', 'valor': 1800.00, 'vip': True},
]
